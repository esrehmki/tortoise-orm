...
class TestManyToManyField(test.TestCase):
    ...
    async def test__add__non__default_schema(self):
        one = await testmodels.M2MOneNonPublicSchema.create(name="One")
        two = await testmodels.M2MTwoNonPublicSchema.create(name="Two")
        await one.two.add(two)
        self.assertEqual(await one.two, [two])
        self.assertEqual(await two.one, [one])
